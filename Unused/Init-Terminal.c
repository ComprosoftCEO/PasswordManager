#include <Terminal.h>
    ...
    */
    ...
    int init_terminal() {
        ...
        /*   Hopefully, this call never fails...  */
        if (stream_makeraw(stdin, &saved)) {return ERROR_STREAM;}
    ...
    int close_terminal() {
        ...
        if (stream_restore(stdin, &saved)) {return ERROR_STREAM;}
    ...
    int init_terminal() {
        ...
        //Disable Control+C and Control+X execution
        if (SetConsoleCtrlHandler(NULL,TRUE) == 0) {return ERROR_CONSOLE;}
        return 0;
    }


    int close_terminal() {
        if (SetConsoleCtrlHandler(NULL,FALSE) == 0) {return ERROR_CONSOLE;}
        return 0;
    }
