...
//Characters allowed in hash string
static const char* hashString = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789";
static const size_t hashStringLen = (sizeof(hashString) / sizeof(hashString[0])) -1;
...
char* Hash8_String(pHash8 h, const char* input, size_t length) {

	pHash8_Obj hash = (pHash8_Obj) h;
	const uint8_t *hashTable = defaultHashTable;
	char* retVal = calloc(1, (sizeof(char) * length) + 1);
	uint8_t hashChar;
	size_t i, j;
	...
	for (i = 0; i < length; i++) {
        hashChar = hashTable[(input[0] + i) % 256];
        for (j = 0; j < strlen(input); j++) {
            hashChar = hashTable[hashChar ^ input[j]];
        }
		retVal[i] = hashString[ hashChar % hashStringLen];
    }
